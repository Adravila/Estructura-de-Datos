...
public:
    ...
    void ingresoPacienteMAL(unsigned int id, unsigned int grav);
...
void Hospital::ingresoPaciente(unsigned int id, unsigned int grav)
{
    ...
    if (current_p == tam_p && current_u == tam_u) // Hospital lleno
    {                                             // Damos de alta al paciente con mejor estado de salud
        Lp.eliminar(Lp.anterior(Lp.fin()));
        --current_p;
    }
    ...
    if (grav > 0 && grav < 6) // Ingresar en la UCI
    {
        ...
        while (p_u != Lu.fin())
        {
            if (grav < Lu.elemento(p_u).grav)
            {
                ...
                {
                    pw = Lu.elemento(Lu.anterior(Lu.fin()));
    ...
    // Si continua, es porque no se ha añadido ningún paciente o ha sido trasladado
    // Vamos a añadirlo en la Planta

    p_p = Lp.primera();
    pa = pw;    // Trasladar paciente a la Planta

    if (p_p == Lp.fin()) // No hay pacientes en la Planta
    {
        Lp.insertar(pa, p_p);
        ++current_p;
        return;
...
int main()
{
    ...
    h.ingresoPaciente(4, 2);
    h.ingresoPaciente(5, 2);
    h.ingresoPaciente(6, 8);
    h.ingresoPaciente(7, 4);
    h.altaPaciente();
    h.altaPaciente();
    h.altaPaciente();
    h.altaPaciente();
    ...
    h.altaPaciente();
    h.altaPaciente();
    h.ingresoPaciente(1, 4);
    h.ingresoPaciente(2, 3);
    h.ingresoPaciente(3, 1);
    h.ingresoPaciente(4, 6);
    h.altaPaciente();
    h.ingresoPaciente(6, 2);
    h.ingresoPaciente(7, 3);
    h.ingresoPaciente(8, 8);
    h.altaPaciente();
    h.ingresoPaciente(9, 9);

    h.pacientesUCI();
    h.pacientesPlanta();
    h.pacientesGravedad(4);
    return 0;
}