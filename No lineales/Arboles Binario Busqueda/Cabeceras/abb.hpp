...
class Abb
{
  public:
    ...
    // Añadido a partir de la práctica 4
    operator Abin<T>() const;
    void insertarArbol(typename Abin<T>::nodo n, Abin<T> &A);
...
Abb<T>::operator Abin<T>() const
{
    ...
    if (r != 0)
    {
        ...
        r->izq.insertarArbol(A.raizB(), A);
        r->der.insertarArbol(A.raizB(), A);
    }
    return A;
}
...
void Abb<T>::insertarArbol(typename Abin<T>::nodo n, Abin<T> &A)
{
    if (r != 0)
    {
        if (n != Abin<T>::NODO_NULO && A.hijoIzqdoB(n) == Abin<T>::NODO_NULO)
        {
            A.insertarHijoIzqdoB(n, r->elto);
            if (!r->izq.vacio())
                r->izq.insertarArbol(A.hijoIzqdoB(n), A);
            if (!r->der.vacio())
                r->der.insertarArbol(A.hijoIzqdoB(n), A);
        }
        else if (n != Abin<T>::NODO_NULO && A.hijoDrchoB(n) == Abin<T>::NODO_NULO)
        {
            A.insertarHijoDrchoB(n, r->elto);
            if (!r->izq.vacio())
                r->izq.insertarArbol(A.hijoDrchoB(n), A);
            if (!r->der.vacio())
                r->der.insertarArbol(A.hijoDrchoB(n), A);
        }
    }
}
